        ...
        <div class="login-main">
          ...
          >
            ...
            <div style=" width: 100%; float: left; margin-bottom: 13px">
              <el-checkbox v-model="isSmsTmp">手机短信验证码登录</el-checkbox>
            </div>
...
import { sendSMS } from '@/api/process/work-order'
// import { sendSMS } from '@/api/process/diy'
...
export default {
  ...
  computed: {
    ...mapGetters(['title', 'logo', 'isLdap', 'isVerifyCode', 'isSms']),
    // 用于校验手机号码格式是否正确
    phoneNumberStyle() {
  ...
  watch: {
    ...
    isSmsTmp: {
      handler: function(val) {
        if (val) {
          this.placeholder_name = '手机号'
          this.placeholder_pass = '验证码'
        } else {
          this.placeholder_name = '用户名'
          this.placeholder_pass = '密码'
        }
      }
    },
    isVerifyCode: {
  ...
  methods: {
    getLoginCode() {
      if (this.phoneNumberStyle) {
        const params = {}
        params.phone = this.loginForm.username
        // 调用获取短信验证码接口
        sendSMS({ phone: params.phone }).then(res => {
          res = res.data
          if (res.status === 500) {
            this.$message({
              message: res.message,
